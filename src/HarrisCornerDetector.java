...
public class HarrisCornerDetector {
	...
	private double[] cornerStrengthRange; //Minimum and maximum values of corner strength values (q)
	...
	public HarrisCornerDetector(ImageProcessor ip, Parameters params) {
		this.ipOrig = ip;
		this.params = params;
		cornerStrengthRange = new double[2];
	}
	...
	public void findCorners(){
		...
		//Get minimum and maximum values of all corner strengthes; hence a range.
		assignCornerStrengthRange();
		if (params.doCleanUp) {
			corners = cleanupCorners(corners);
		}
	}
	
	private void assignCornerStrengthRange()
	{
		double[] cornerStrengths = new double[corners.size()];
		int i = 0;
		for(Corner c : corners)
		{
			cornerStrengths[i] = c.q;
			i++;
		}
		
		cornerStrengthRange = Util.getMinMax(cornerStrengths);
	}
...
//		FloatProcessor Iy = (FloatProcessor) ipOrig.convertToFloat();
	...
	public ImageProcessor showCornerPoints(ImageProcessor ip, int nmax){
		...
		//Convert the image into RGB again to prepare for color corner drawing
		ColorProcessor ipColor = (ColorProcessor)ipResult.convertToRGB();
		//draw corners:
		int n = 0;
		for (Corner c: corners) {
			drawCorner(ipColor, c);
			n = n + 1;
			if (nmax > 0 && n >= nmax) 
				break;
		}
	...
	private void drawCorner(ImageProcessor ip, Corner c){
		...
		ip.setColor(getColor(c.q));
		//size += getAddedSize(c.q);
		width += getAddedWidth(c.q);
		ip.setLineWidth(width);
		ip.drawLine(uu - size, vv, uu + size, vv);
		ip.drawLine(uu, vv - size, uu, vv + size);
	}
	
	private double getAddedWidth(double value)
	{
		double[] destRange = new double[2];
		destRange[Util.MIN] = 0;
		destRange[Util.MAX] = 2;
		double addedWidth = Util.scale(cornerStrengthRange, destRange, value);
		
		return addedWidth;
	}
	
	private int getAddedSize(double value)
	{
		double[] destRange = new double[2];
		destRange[Util.MIN] = 0;
		destRange[Util.MAX] = 6;
		double addedSize = Util.scale(cornerStrengthRange, destRange, value);
		
		return (int)Math.round(addedSize);
	}
	...
	private Color getColor(double value)
	{
		double[] destRange = new double[2];
		destRange[Util.MIN] = 0;
		destRange[Util.MAX] = 0.5; // 0 to 180 degrees of Hue
		float strength = (float)Util.scale(cornerStrengthRange, destRange, value);
		float hue = 0.5f - strength; //So that greater strength will go close to red side, and lower strength goes toward blue side
